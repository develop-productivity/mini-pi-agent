...
这是 [agent_demo](https://github.com/develop-productivity/agent_demo)(一个 20 天从零手写完整 agent 框架的学习记录)之后的复现练习:不看那份完整实现,只凭对 pi 的 `agent-loop` 源码的理解、加上对真实 API 文档的查阅,重新写一遍最小可运行版本。
...
| `Cannot read properties of undefined (reading '0')` | DeepSeek 返回的是一个错误 JSON(比如缺字段、key 不对),代码却直接假设它是成功响应 | **网络请求必须先检查 `res.ok`,不能假设它一定成功**(Day 1 的教训,新项目里又踩了一次) |
...
## 跟 [agent_demo](https://github.com/develop-productivity/agent_demo) 的关系

那是一个 20 天、按天记笔记的完整实现,从工具调用讲到会话隔离、可观测性、多租户。这个仓库是**独立**写的复现练习——不引用那边的任何代码,只作为"读完笔记、自己重新想一遍"的验收。两边如果在某个设计上不一样,大概率是因为这边是从零推导出来的,不是抄的。
